...
| Colonne d'import `meter_reference` | D-167 (numéro de séquence du segment) / D-279 (INITIAL ou COURANT) | D-279 : INITIAL crée le premier compteur, COURANT ajoute au compteur en service | D-279 est la décision dédiée au vocabulaire d'import ; aucun remplacement implicite. |
...
| Conservation des exports asynchrones | D-272 (7 jours) / D-273 (24 heures) | 24 heures | La durée la plus courte limite l'exposition de données exportées. |
| Échéances d'entretien | D-200 (date de prochain changement de statut matérialisée) | Statut matérialisé recalculé à chaque événement et par le rattrapage périodique, sans colonne `nextStatusChangeOn` | Une seule source de statut ; le rattrapage borne le délai de bascule. |
| Intervention sans coût | D-206 (dépense à 0) / D-233 (montant strictement positif) | Aucune dépense à 0 ; l'intervention porte l'état de coût `SANS_COUT` | D-233 interdit un montant nul dans le registre des dépenses. |
| Référence de dépense manuelle | Index d'expression unique | Contrôle dans une transaction sérialisable (comparaison en minuscules) | Même garantie sous concurrence, testée par des requêtes simultanées. |
| Historisation quotidienne télématique | D-181 / D-188 | Premier échantillon du jour local | Aucun échantillon n'est inventé à minuit exact. |
...
| Permission costs.write | D-266 (costs.write implique costs.read) | Permissions explicites : costs.write n'implique pas costs.read ; l'administrateur accorde costs.read au besoin. Sans costs.read, tickets de plein et factures de dépense ne sont pas téléchargeables | CDC 2.2 : « consultation des coûts soumise à permissions explicites ». |
| Tentatives d'envoi d'e-mail | D-261 (8 délais) / D-263 et schéma (8 tentatives) | 8 tentatives au total (délais 1, 2, 4, 8, 16, 32 et 60 min entre elles), puis ABANDONNE | Cohérent avec `maxAttempts = 8` du schéma. |
| Transfert avec relevés EN_ATTENTE ou pleins SOUMIS | D-119 (bloquants) / D-125 (avertissements à acquitter) | Avertissements à acquitter explicitement (acknowledgeWarnings) | D-125, plus récent et dédié à l'assistant de transfert ; aucun objet n'est perdu. |
| Alerte GPS_SYNCHRO_EN_ECHEC | D-247 (une par société, visible du chef) / D-248 (administrateur seul) | Visible des chefs de parc des sociétés couvertes et de l'administrateur | CDC 14.6 : le chef consulte l'état de synchronisation de ses sociétés. |
...
| Préférence de gravité minimale des e-mails | D-245 (CRITIQUE par défaut) | Aucune préférence créée implicitement ; la gravité minimale est choisie explicitement par l'utilisateur | Le défaut de colonne (URGENT) n'est jamais appliqué sans choix explicite. |
...
| Visibilité des lots d'import | D-277 (non précisé) | L'administrateur voit tous les lots ; le chef de parc voit les lots qu'il a téléversés | Un lot peut concerner plusieurs sociétés ; chaque ligne reste contrôlée sur le périmètre de celui qui importe. |
| Purge des lots d'import à 90 jours | D-279 (purge de ImportRow.data) | Les valeurs brutes des lignes et le fichier source privé du lot sont supprimés 90 jours après la fin du lot (confirmation ou abandon) ; statut, messages d'erreur et rapport sont conservés. Abandon automatique après 7 jours, audité comme action système | Le fichier source contient les mêmes données brutes que les lignes ; les conserver viderait la purge de son sens. |
| Partitions des échantillons télématiques | Migration initiale (fonction ensure_month_partitions) | Le worker crée chaque jour les partitions des trois mois suivants ; le mois courant reste dans la partition par défaut si elle contient déjà des lignes de ce mois | Créer la partition d'un mois déjà présent dans la partition par défaut échouerait. |
| Planification du worker | CDC 14.1 | Tâches à bail unique : rattrapage des alertes, outbox e-mail, récapitulatif quotidien, purge quotidienne, file de jobs (exports, toutes les 5 s, purge des exports expirés toutes les 15 min), synchronisation télématique (chaque minute, aucun appel sans fournisseur actif et activé), ingestion des lots webhook télématiques (toutes les 10 s, rien sans lot en file), rétention quotidienne. Le worker refuse de démarrer en production si un fournisseur SIMULATEUR est actif | Un seul worker exécute chaque tâche ; un autre la reprend à l'expiration du bail. |
| Référence de repli des échantillons télématiques | D-195 (l'adaptateur calcule une référence synthétique `syn:`) / D-184 | Les adaptateurs transmettent sourceReference nul quand le fournisseur n'en donne pas ; le service d'ingestion calcule la référence de repli `fp:` (empreinte unité, instant, valeur convertie, nature) | Une seule implémentation de l'empreinte, commune à tous les canaux ; même idempotence (index unique fournisseur, unité, référence). |
| Contrat OpenAPI versionné | D-306 (packages/contracts/openapi.json, test contracts.int.spec.ts, échec en CI) | docs/openapi.json généré par `pnpm --filter @parc-auto/api openapi:export` (même configuration que l'API servie) et test de dérive unitaire apps/api/src/openapi-drift.spec.ts exécuté par `pnpm test` ; les réponses d'erreur (enveloppe code/message/fieldErrors) ne sont pas décrites opération par opération | Le dépôt n'a pas de CI hébergée ; le contrôle bloque la suite de tests locale. Les types web restent écrits à la main (lib/*-types.ts). |
| Jeu de démonstration | D-324 (packages/db/src/seed-demo.ts) | apps/api/src/cli/seed-demo.ts : passe par les services métier de l'API (références, audit, alertes, règles), refusé en production et sur une base non vide sans --reset | Seul un passage par les services garantit des données cohérentes avec les règles. |
| Ticket carburant du responsable habituel | D-226 (utilisation obligatoire) / D-268 (paramètre drivers.allowHabitualVehicleSubmissions ouvrant les actions conducteur sur le véhicule dont il est responsable habituel) | D-268 : paramètre activé, le responsable habituel actif soumet un ticket hors utilisation (aucune utilisation EN_COURS), daté de moins de fuel.driverLateSubmissionDays ; paramètre désactivé, la règle D-226 s'applique seule | D-268 est la décision dédiée à « Mon véhicule », postérieure et plus spécifique. |
| Relevé et signalement du responsable habituel | D-216 (signalement sur l'utilisation en cours ou terminée depuis moins de incidents.driverLateDeclarationHours) / D-268 (actions conducteur ouvertes sur le véhicule dont il est responsable habituel) | D-268 : paramètre activé et aucune utilisation EN_COURS, le responsable habituel actif soumet un relevé (EN_ATTENTE) et signale un problème survenu depuis moins de incidents.driverLateDeclarationHours, sans utilisation rattachée ; une utilisation EN_COURS ou terminée dans ce délai reste prioritaire ; autre véhicule ou paramètre désactivé : 404 (403 pour un relevé sur un véhicule lisible). Règle unique : DriverSubmissionService (GET /driver-submissions/vehicles), appliquée aussi au ticket carburant | Même cible pour les trois actions de « Mon véhicule », et même délai que la déclaration après restitution. |
| Canal RAPPORT : unités vues en synchronisation | D-296 (liste des unités au plus une fois par heure) | Pour un canal à liste partielle, les unités lues par la synchronisation sont enregistrées avant l'acquittement des fichiers (run de découverte tracé) | Sinon une unité présente seulement dans des fichiers déjà acquittés ne serait jamais proposée. |
| Ticket de plein et remplissages détectés | 8.5, D-242 | Un plein ne justifie qu'un seul remplissage détecté : les pleins déjà rapprochés sont exclus des candidats, y compris au rapprochement tardif | Un même ticket ne peut pas couvrir deux remplissages. |
| Runs IGNORE (coupe-circuit, quota) | D-297 | Tracés hors bail : avec plusieurs répliques du worker, un même créneau ignoré peut être tracé deux fois (sans effet sur les données) | Déploiement cible : un seul worker (docker-compose.prod.yml). |
| Catalogue initial (6.1, 7.1) | CDC 6.1 « le catalogue … comprend au départ … », 7.1 « types configurables par objet : assurance, … » ; aucune décision sur sa livraison | Catalogue livré dans le code (`apps/api/src/domain/initial-catalog.ts`) et **installé à la demande de l'administrateur** (`POST /maintenance-types/initial-catalog`, `POST /document-types/initial-catalog`, boutons « Installer le catalogue initial » de `/entretiens` et `/documents`), en production comme en démonstration, jamais par migration ni au démarrage. Seuls les éléments absents sont ajoutés (même code, ou même libellé — pour le même objet côté documents —, actifs ou archivés : ignorés) ; aucun élément existant n'est modifié ni réactivé ; `ON CONFLICT DO NOTHING` sur le code unique sous concurrence ; Idempotency-Key facultative ; audit par élément créé et de l'installation. Opérations sans intervalle ; types de documents facultatifs et non bloquants avec les préavis paramétrés. Le permis de conduire n'est pas installé comme type de document (DriverPermit, D-133) | Rien n'est imposé à une organisation déjà paramétrée (6.1 : aucun intervalle universel ; 3.3 : pas de règle juridique inventée) et l'installation reste rejouable sans effet. |
| Prévisualisation des impacts (17.1) | 17.1 « modifier un intervalle d'entretien … prévisualiser les impacts avant validation » | Prévisualisation serveur sans écriture (transaction annulée) pour la modification d'un plan (`PATCH /maintenance-plans/:id`, `preview=true`) et pour l'application d'un modèle (`POST /maintenance-templates/:id/apply`, `preview=true` : impact plan par plan avant/après avec la version de chaque plan existant, sans consommer la clé d'idempotence) ; l'écran d'application présente l'impact avant confirmation dès qu'un plan existant serait mis à jour, et la confirmation renvoie les plans et versions présentés (`expectedPlanVersions`) : si l'impact a changé entre-temps (plan modifié, créé ou désactivé), 409 `VERSION_OBSOLETE` et rien n'est enregistré (verrou optimiste de l'aperçu). Les préavis d'entretien par défaut (`maintenance.noticeKm`, `maintenance.noticeDays`) sont copiés dans chaque plan à sa création : les modifier n'a aucun impact sur les plans existants, donc rien à prévisualiser (testé) | La phrase du CDC vise la modification d'un intervalle ; un paramètre par défaut ne s'applique qu'aux plans créés ensuite. |
| Alertes à la clôture d'intervention (6.4, D-206) | 6.4 « clôturer dans une transaction : … dépense liée et alertes » ; D-206 (coût saisi plus tard) | Les alertes des plans du véhicule sont réévaluées dans la transaction sérialisable de clôture (et de réouverture) ; les notifications e-mail partent après validation. La dépense est créée dans la même transaction quand le coût est connu ; sinon l'intervention reste « coût à saisir » et l'unique dépense est créée par la saisie du coût (D-206) | Tout ou rien, alertes comprises (un échec d'écriture d'alerte annule la clôture, testé) ; une notification n'est jamais envoyée pour une clôture annulée. |
| Pièces jointes d'une intervention (15.3) | 15.3 « plans, fournisseurs et documents hors périmètre refusés » | À la clôture et à la saisie du coût, un fichier (rapport, photo du relevé d'exécution, facture) n'est rattachable que s'il a été téléversé pour la société historique de l'intervention (ou sans société par l'administrateur) : sinon 404, même si l'utilisateur voit les deux sociétés | Le périmètre est celui de l'intervention, pas seulement celui de l'utilisateur ; aucun fichier d'une autre société ne change de société en silence. |
| Unité volontairement sans véhicule (remorque, boîtier de rechange) | D-249 (alerte INFO levée par un nouvel état de mapping IGNOREE, `TelemetryMappingStatus.IGNOREE`) | L'état est porté par l'unité : `TelemetryUnit.ignoredAt`, `ignoredById`, `ignoredReason` (CHECK : tous nuls, ou date et motif non vide). `POST /telemetry/units/:id/ignore` (motif) et `unignore` : administrateur ou chef d'une société couverte ; 409 si une association confirmée est en cours ; propositions en attente rejetées ; plus de proposition (motif IGNOREE à la découverte) ni d'alerte GPS_UNITE_NON_MAPPEE ; catégorie IGNOREES ; audit `telemetrie.unite.ignoree` / `telemetrie.unite.reprise` | `TelemetryVehicleMapping` exige un véhicule : un statut d'association IGNOREE devrait porter un véhicule fictif. |
| Jeu de démonstration (19.2) | D-324 (fichier prévu `packages/db/src/seed-demo.ts` ; refus en production ; dates relatives à l'exécution) | `apps/api/src/cli/seed-demo.ts` (`pnpm seed:demo`, compilé avec l'API) : organisation et premier administrateur écrits directement comme `create-admin` (trace d'audit système), tout le reste par les services métier de l'API sous le contexte d'autorisation réel de l'administrateur, des chefs et des conducteurs. Refus si `NODE_ENV=production` (avant toute connexion) et sur base non vide ; `--reset` (hors production) ne vide qu'une base ne contenant que l'organisation `DEMO`. Mot de passe commun : `DEMO_PASSWORD` contrôlé, sinon généré et affiché une seule fois. Les étapes datées (relevés, remises, restitutions, pleins, remplacement de compteur, entretien réalisé) s'exécutent à leur heure via l'horloge du chargement : alertes levées et résolues dans l'ordre réel, aucune alerte de fraîcheur fictive | Mêmes règles, références, audit et alertes que depuis l'interface ; `packages/db` ne dépend pas de l'API. Une base portant d'autres organisations (données réelles, `NODE_ENV` absent) n'est jamais vidée. |
| Réception webhook fournisseur (14.4, R-14.4-02, R-14.4-X01) | D-298 (webhook authentifié par HMAC, désactivé par défaut, qui met en file un job ; seul le worker crée les relevés) | Type `WEBHOOK_GENERIQUE` (canal `WEBHOOK`) activé explicitement ; `POST /api/v1/telemetry/webhooks/:providerId` public, sans session ni cookie (garde CSRF non applicable à cette seule route, `@SignedWebhook()`), signature HMAC-SHA256 de « horodatage.corps brut », tolérance ± 300 s, rejeu d'une même requête signée refusé (409 `WEBHOOK_REJOUE`, index unique), corps limité à 1 Mio (413), débit par fournisseur compté en base sous verrou (429, `Retry-After`) ; file dédiée `TelemetryWebhookDelivery` (lot normalisé, champs non prévus non conservés) plutôt que `Job` ; ingestion par la tâche `telematique-webhooks` (10 s) via la même exécution que la synchronisation (run `WEBHOOK` par société activée, même ingestion idempotente) | Une table dédiée porte les invariants en contraintes (anti-rejeu, cohérence état/verrou, empreinte) et sert le suivi de l'administrateur ; la même exécution garantit les mêmes contrôles et l'absence de doublon quand le fournisseur renvoie un lot. |
| Secret de signature d'un webhook | CDC 14.6 (secret jamais renvoyé par l'API) / nécessité de communiquer le secret au fournisseur | Secret `SIGNATURE_WEBHOOK` généré dans le navigateur de l'administrateur (Web Crypto, 32 octets), affiché une seule fois puis déposé en écriture seule et chiffré comme les autres secrets ; un nouveau dépôt est une rotation : l'ancien reste accepté pendant `rotationOverlapHours` (24 h par défaut, `TelemetryCredential.expiresAt`, jamais sur un secret actif) puis est supprimé ; « Révoquer » supprime tout immédiatement | L'API ne renvoie jamais un secret, même à sa création ; le recouvrement évite de perdre des lots pendant la bascule chez le fournisseur. |
| Lot webhook d'un fournisseur non actif ou sans société activée | D-298 / D-101, D-295 | Émetteur authentifié : 422 `FOURNISSEUR_INACTIF` ou `TELEMETRIE_DESACTIVEE` à la réception, rien n'est déposé ; devenu non actif entre la réception et le traitement : lot `IGNORE`, jamais ingéré ni repris à la réactivation ; aucune synchronisation manuelle pour ce canal (422 `SYNCHRO_PAR_WEBHOOK`) | Le fournisseur est informé du refus ; aucune donnée d'une période de désactivation n'est ingérée. |
| Paramètres `pagination.*` (17.1) | R-17.1-X01 (chaque valeur du tableau 17.1 modifiable depuis /administration) / CDC 15.1 (« pagination bornée », contrat OpenAPI) | Valeurs fixes du produit : 25 lignes par défaut, 100 au plus par requête, affichées dans Administration › Paramètres (« Valeur fixe ») mais non modifiables (`PUT /settings/pagination.*` → 422 `PARAMETRE_NON_MODIFIABLE`, descripteur `fixed`) ; `PAGINATION` en découle | Le CDC 17.1 propose ces valeurs initiales sans exiger qu'elles varient par organisation ; elles bornent le contrat de l'API documenté dans OpenAPI (`maximum: 100`) et les écrans ; une valeur modifiable mais sans effet serait trompeuse. |
| Durée de session (16.1, 17.1) | Variable d'environnement `SESSION_TTL_HOURS` (première implémentation) / paramètre `session.ttlHours` | Paramètre `session.ttlHours` seul (12 h par défaut, 1 à 72 h, administrateur, audité), lu à chaque connexion ; `SESSION_TTL_HOURS` supprimée de l'environnement ; une session ouverte garde son échéance | CDC 16.1 « délai de session proposé de douze heures, configurable » : une seule source de vérité, modifiable sans redéploiement et tracée. |
| Langue, fuseau, devise et décimales (17.1) | R-17.1-X01 (paramètres versionnés dans Settings) | Colonnes de l'organisation : fuseau modifiable (Administration › Organisation, verrou optimiste, audit) ; langue `fr`, devise TND et 3 décimales fixes en V1 | CDC 10.1 (application en français, pas d'interface arabe imposée) et 17.1 (TND, trois décimales) : aucune autre valeur n'est prévue en V1 ; ces valeurs portent sur l'organisation entière et ne se surchargent pas par société. |
| Structure de traduction (10.1) | R-10.1-03 (libellés externalisés dans un catalogue, `apps/web/lib/i18n.ts` prévu) | Libellés métier externalisés dans `packages/contracts` (statuts, énumérations, libellés et bornes des paramètres), partagés par l'API et le web ; `lang="fr"` ; les textes propres à chaque écran restent dans les composants | Une interface arabe n'est pas demandée en V1 ; un catalogue partiel créé sans seconde langue ne serait pas vérifiable. L'externalisation des textes d'écran est à faire si une seconde langue est commandée (écart ouvert, R-10.1-03). |
| Tri au choix des listes (10.1) | R-10.1-04 (tri au choix sur toutes les tables) | Tri au choix (en-têtes de colonnes accessibles, `tri`/`sens` dans l'URL, liste autorisée côté API, ordre stable) sur les listes de référence : véhicules, conducteurs, utilisateurs, sociétés, plans d'entretien, interventions, dépenses ; les autres listes gardent un ordre métier fixe et annoncé (chronologique, urgence, échéance) | Les listes d'événements et files de travail ont un ordre imposé par leur usage (file de validation, alertes par gravité) ; l'extension aux autres listes reste ouverte (R-10.1-04). |
| Validation d'une dépense manuelle (15.2, 10.2) | CDC 15.2 (« /expenses : saisie, validation, annulation/correction ») ; 8.2 (soumission conducteur = brouillon à valider) ; D-229 (dépense immuable) | La validation s'applique aux objets soumis qui produisent une dépense : plein de conducteur (`POST /fuel-entries/:id/validate`) et intervention (clôture), chacun générant sa dépense de synthèse. Une dépense manuelle n'est saisie que par un compte `costs.write` du personnel et est enregistrée `VALIDEE` dès la saisie : aucun état « à valider », aucune route `POST /expenses/:id/validate` | Seules les soumissions des conducteurs sont des brouillons dans le CDC (8.2), et les conducteurs n'accèdent pas au registre ; un état intermédiaire sur une saisie du personnel ajouterait une double saisie sans contrôle supplémentaire. Toute erreur se traite par une nouvelle version ou une annulation motivée, tracées et réservées au chef et à l'administrateur (D-229). |
| Santé du worker et surveillance externe (16.3) | D-315 (`/health/worker` en 503 au-delà de 2 min ; `/health/ready` sans le worker ; ping sortant « dead man's switch » `HEALTHCHECK_PING_URL`) / première implémentation (seuil de 5 min, worker affiché dans `/health/ready` sans route dédiée) | `/health/ready` : 200 « pret » si la base (joignable, pas en lecture seule) et le stockage (répertoire lisible et inscriptible) sont accessibles, 503 « degrade » sinon ; `checks.worker` y reste informatif. `/health/worker` : 200 « actif » si un battement date de 2 min au plus (battement toutes les 30 s, règle unique `WORKER_HEARTBEAT_STALE_MS`), 503 « arrete » ou « inconnu » (base injoignable). Aucun ping sortant : la supervision hébergée hors du VPS sonde `/health/ready` et `/health/worker` (docs/installation.md §6) ; `restore-test.sh` attend les deux | Un worker arrêté ne rend pas l'API indisponible (D-315), mais se voit de l'extérieur sans dépendre d'un appel sortant ni d'un service tiers supplémentaire ; une panne totale du VPS fait échouer les deux sondes. |
| Tableau de bord par véhicule (11.1) | CDC 11.1 (« par société, période et véhicule ») ; aucune décision | `GET /dashboard?vehicleId=` : tous les indicateurs et leurs listes justificatives (même paramètre `vehicleId`, `vehicule` pour les écrans de liste ; la liste des véhicules passe alors par la route de l'API) portent sur ce véhicule ; il doit être géré par une société du périmètre, et par la société demandée le cas échéant, sinon 404 | Un véhicule transféré se filtre dans le périmètre de sa société actuelle ; ses coûts historiques restent imputés à l'ancienne société (rapports de dépenses), jamais réattribués. |
| Contrôle de persistance après redémarrage (17.2, T30) | R-17.2-07 (`scripts/tests/restart-persistence.sh`) | Projet compose isolé aux volumes neufs (postgres, migrate, api, worker, web ; secrets générés à l'exécution), données créées par l'API réelle, `down` puis `up`, comparaison des comptes de lignes, de la session, du véhicule et de l'empreinte de la pièce jointe ; le reverse proxy n'est pas démarré | Le proxy publie les ports 80/443 de l'hôte (conflit avec une production sur la même machine) et ne porte aucune donnée métier ; exécuté sur les images construites (`docker compose -f docker-compose.prod.yml build`), jamais simulé. |
| Refus d'un import au-delà de 5 Mo (12.1, 17.1) | Première implémentation (plafond multer : 413 `CONTENU_TROP_VOLUMINEUX` générique) | Même refus métier qu'au-delà de la limite paramétrée : 422 `FICHIER_TROP_VOLUMINEUX` avec message et erreur de champ en français ; rien n'est conservé | Un seul code pour une même règle, quel que soit le composant qui la détecte. |
| Envoi hors connexion (1.3, 10.3) | Première implémentation (mutations TanStack Query en mode réseau par défaut : mises en pause hors connexion, puis rejouées automatiquement au retour du réseau) | Mutations en mode « always » (`apps/web/lib/query-client.ts`) : hors connexion, l'envoi échoue aussitôt avec « Impossible de joindre le serveur… rien n'a été enregistré » ; la saisie reste dans le formulaire et ne part que sur une nouvelle action de l'utilisateur (même clé d'idempotence) | La V1 exclut le fonctionnement hors ligne avec synchronisation (1.3) et exige une erreur explicite hors connexion (10.3) : un envoi différé et rejoué seul contredisait les deux. |
| Rapports regroupés paginés en base (17.2) | R-17.2-06 (« aucun historique complet chargé pour une page, pagination en base ») / principe de règle de calcul unique | Les vues regroupées ou calculées des rapports (dépenses par catégorie, véhicule, société, fournisseur et détail ; carburant par véhicule ; retards ; qualité des relevés ; durées d'immobilisation et ventilation par cause ; expirations documentaires ; inventaire filtré par statut opérationnel) sont filtrées, regroupées, triées et découpées par PostgreSQL (`report-sql.ts`, SQL paramétré) ; les traductions SQL de `returnDelay`, de la limite `staleBefore`, de `unionWithinPeriod` (multiranges `range_agg`) et de `computeDocumentStatus` ne servent qu'à filtrer, trier et découper ; les valeurs affichées de la page restent calculées par les règles uniques du domaine (`summarizeLedger` sur des lignes agrégées, `returnDelay`, `computeFreshness`, `unionWithinPeriod`, `complianceRowsFor`) ; tri des libellés en collation ICU racine (`COLLATE "und-x-icu"`, même ordre que `localeCompare`) | Une page de 25 lignes ne charge plus toute la période (lignes renvoyées par la base mesurées pour chaque vue par `reports-pagination.int.spec.ts`, proportionnelles à la page ; le rapport coûts et distances découpe encore en mémoire le référentiel des véhicules candidats et de leurs transferts) ; l'équivalence des traductions est vérifiée par `reports-pagination.int.spec.ts` (mêmes ensembles que `/documents/compliance`, `/vehicles?freshness=`, `/vehicles?operationalStatus=`, `returnDelay` et `unionWithinPeriod` recalculés, pages de 7, 25 et 100 identiques) et les réponses de l'ancienne et de la nouvelle implémentation ont été comparées sur 822 requêtes (toutes vues, filtres et fenêtres) du jeu de dimensionnement à l'échelle 0,1 : aucun écart (la réécriture ultérieure du SQL des documents, par jointures de hachage, est couverte par le même test d'intégration). |
| Jeu et test de dimensionnement (17.2) | R-17.2-01, R-17.2-03, R-17.2-04 | `apps/api/src/cli/seed-load.ts` (hypothèse du CDC : 3 sociétés, 500 véhicules, 1 000 conducteurs, 300 000 relevés, 50 comptes ; refus en production et sur base non vide ; insertion `createMany` par lots) et `scripts/tests/load-test.mjs` (50 sessions réelles, lectures paginées et mutations simples, p50/p95/p99/max) ; mesures, matériel et limites dans `docs/performance.md` ; index de listes ajoutés (`20260924223000_lot_e_perf_list_indexes`) | Les statuts d'entretien matérialisés ne sont pas écrits par le jeu (calculés par la règle unique à la première lecture du jour ou par le worker) ; ni transfert de société ni télématique dans le jeu (volumétrie F11 non exercée) ; la mesure est faite sur une machine partagée (voir `docs/performance.md`) : le seuil des mutations n'y est pas tenu, ni celui des lectures pour le rapport carburant (p95 3,9 s, environ 14 requêtes par véhicule de la page), malgré un p95 agrégé des lectures de 1,3 s. Les optimisations décrites au § 7 de `docs/performance.md` (recalculs dépendants groupés, remise et restitution en READ COMMITTED sous verrous, consommations du rapport carburant groupées, matérialisation des plans par lots) ont été faites après cette mesure : la mesure de charge est à refaire, ces écarts restent ouverts d'ici là (R-17.2-04). |
| Isolation de la remise et de la restitution (13.3, 17.2) | D-138 (verrous véhicule puis conducteur, `SELECT … FOR UPDATE`, READ COMMITTED) / première implémentation (transaction sérialisable reprise trois fois, puis 409 `CONCURRENCE`) | D-138 : `PrismaService.lockedReadCommitted` (READ COMMITTED explicite ; reprise bornée sur interblocage ou conflit d'écriture, puis 409 `CONCURRENCE`). Véhicule puis conducteur verrouillés `FOR UPDATE` avec une nouvelle version de leur ligne, sans changement de valeur (`lockVehicleForWrite`, `lockDriverForWrite`) : une transaction encore sérialisable (archivage, réservation, immobilisation, transfert, prolongation…) qui attendait ce verrou lit un instantané antérieur à la remise ; la nouvelle version la fait échouer en conflit de sérialisation et reprendre avec un instantané à jour (test D-129 archivage contre remise). L'invariant tient par les verrous et les contraintes : (1) **une utilisation ouverte par véhicule et par conducteur, aucune possession qui chevauche** (T04, T05) : remise et restitution, seules écritures d'utilisation, verrouillent le véhicule puis le conducteur ; les contrôles de départ (utilisation ouverte, restitution postérieure) sont relus après ces verrous et voient donc toute écriture validée avant ; index uniques partiels `usage_one_open_per_vehicle` et `usage_one_open_per_driver` en garde finale (409 `VEHICULE_DEJA_EN_UTILISATION` / `CONDUCTEUR_DEJA_EN_UTILISATION`) ; (2) **réservations** : création et modification verrouillent les mêmes véhicule et conducteur, les réservations confirmées d'autrui sont relues sous verrou ; contraintes d'exclusion entre réservations ; la réservation à convertir est lue `FOR UPDATE` (annulation, non-présentation et expiration verrouillent la réservation) : conversion et annulation se suivent, jamais les deux ; (3) **relevé de départ et de retour** : service d'ingestion unique sous le verrou du véhicule, que prennent toutes les écritures de relevés et de compteurs (saisie, validation, rejet, correction, plein, intervention, transfert, télématique), unicité du relevé accepté par instant en garde finale (409 `CONFLIT_MEME_INSTANT`) ; (4) **immobilisation, cycle de vie, transfert du véhicule, statut du conducteur** : écrits sous le verrou du véhicule ou du conducteur ; (5) **restitution** : utilisation verrouillée après véhicule et conducteur, statut et version relus sous verrou (retours concurrents : un seul aboutit, test 13.3) ; (6) **pièce jointe** : rattachement par écriture conditionnelle (encore libre et non supprimée au moment de l'écriture). Documents, permis et catégorie sont lus une fois, sans écriture croisée : le résultat est celui d'une exécution en série. Relevé libre, prolongation, régularisation et correction restent sérialisables | En sérialisable, les verrous de prédicat posés par page sur les petits index partiels faisaient échouer entre elles des remises de véhicules différents (26 refus `CONCURRENCE` sur 511 remises et restitutions à la mesure du 24/09/2026), et une transaction qui attendait le verrou du véhicule lisait un instantané antérieur à son détenteur. Tests de concurrence réelle : T04, T05, T33, 13.3, remises et restitutions simultanées de couples distincts, conversion contre annulation ; entrelacements forcés (verrou tenu par une transaction de test, attente observée dans `pg_stat_activity`, `usages-isolation.int.spec.ts`) : archivage, désactivation du conducteur, réservation et relevé décidés pendant une remise non validée, immobilisation validée pendant que la remise attend, restitution validée pendant que la remise du même conducteur attend, même photo jointe à deux remises, paramètres propres à chaque organisation et à chaque requête. Règle à tenir : toute nouvelle écriture qui peut invalider un contrôle de départ prend le verrou du véhicule ou du conducteur. |
| Associations unité ↔ véhicule sans chevauchement (14.5, R-14.5-X01) | Première implémentation (index « une association ouverte », date d'effet bornée par le service : 422 `DATE_EFFET_TROP_ANCIENNE`) / R-14.5-X01 (contrainte d'exclusion, 409) | Contraintes d'exclusion SQL `telemetry_mapping_no_overlap_unit` et `_vehicle` sur [validFrom, validTo[ des associations CONFIRME et CLOTURE, CHECK `telemetry_mapping_period_valid` ; une date d'effet antérieure à la fin de l'association précédente de l'unité ou du véhicule, ou une violation concurrente, donne 409 `PERIODE_UNITE_CHEVAUCHEMENT` / `PERIODE_VEHICULE_CHEVAUCHEMENT` ; les autres bornes (reprise initiale, compteur, entrée dans la société, date future) restent en 422 | Un chevauchement est un conflit d'état (CDC 15.1) ; la base le garantit même entre décisions concurrentes. |
| Seuils carburant par véhicule (8.5, 17.1) | D-238 (`fuel.refill`, `fuel.drop`, `fuel.ticketMatch` : groupe > société > véhicule) / D-240 (surcharges par véhicule) | Table `VehicleFuelThresholds` : seuils de baisse à l'arrêt et de remplissage (litres, pourcentage, fenêtre) propres au véhicule, prioritaires sur ceux de la société ; `GET`/`PUT /telemetry/vehicles/:id/fuel-thresholds` (chef de parc ou administrateur, motif, verrou optimiste, audit ; tous nuls = retour à la société). Rapprochement avec le ticket et pas des échantillons restent des paramètres de la société ou du groupe | Le chef ajuste la sensibilité de la mesure d'un véhicule ; la tolérance du ticket est une règle de saisie commune à la société. |
| Consommation télématique en parallèle (8.5, R-8.5-10) | D-234, D-236 (sonde : niveau après A − niveau avant B + remplissages de ]A, B] ; N/D au-delà d'un trou de 60 min ou sous 90 % de couverture) | Mêmes intervalles A → B et mêmes kilomètres que la consommation déclarée, ajoutés à `GET /vehicles/:id/consumption` (champ `telematics`, absent sans mesure exploitable) : compteur CAN = dernier − premier échantillon de [A, B] ; sonde = pic du remplissage détecté rapproché du plein A − creux du remplissage rapproché du plein B + remplissages intermédiaires mesurés (pic − creux) ; N/D motivé si un remplissage A ou B n'est pas détecté, trou > 60 min, couverture < 90 %, volume inconnu, bilan négatif ou mesures de deux boîtiers dans l'intervalle (changement de boîtier : `BILAN_INCOHERENT`) ; compteur CAN préféré à la sonde parmi les mesures reçues (une nature déclarée jamais reçue ne masque pas l'autre), jauge CAN jamais utilisée ; écart en % sur les seuls intervalles comparables | L'heure d'un ticket est approximative : borner au plus près du remplissage mesuré évite de compter une partie du plein comme consommation. |
| Aide télématique de la remise et de la restitution (5.6, R-5.6-17) | Première implémentation (dernier état de l'unité, quelle que soit sa date ; distance GPS brute proposée pour une association DISTANCE_GPS) | Dernière valeur automatique de la nature retenue par l'association en cours, observée depuis sa date d'effet (`GET /vehicles/:id/odometer`, aperçu de remise) : échantillon COMPTEUR_CAN ; pour DISTANCE_GPS, dernière estimation acceptée calibrée sur une référence de cette association, ramenée au compteur physique de son segment ouvert (jamais la distance brute du boîtier) ; aucune aide sans association ouverte, pour une association sans kilométrage, sans référence manuelle du boîtier ou si le module est désactivé pour la société | Un boîtier réutilisé ne doit jamais proposer la valeur observée sur son véhicule précédent ; une distance GPS n'est jamais présentée comme un compteur (5.6) ; module désactivé = régime manuel (14.3). |
| Consommation théorique du fournisseur (8.5, annexe A) | CDC 8.5 (« n'est pas importée comme mesure ») | Aucune nature théorique dans le modèle : association, simulateur, webhook et configurations Traccar, Wialon et rapports refusent toute nature hors `NIVEAU_CAN`, `NIVEAU_SONDE`, `CONSOMMATION_CAN` (422) ; seuls l'attribut, le capteur ou la colonne désignés sont lus | Limite : si l'attribut désigné est lui-même calculé par la plateforme à partir d'un taux, le logiciel ne peut pas le reconnaître ; la question 5 de l'annexe A en décide avant configuration. |
...
Réouverture TERMINEE → EN_COURS (chef ou administrateur, motif, audit) : dans la même transaction, annulation traçable de la dépense, recalcul des bases sans l'intervention et réévaluation des alertes. Le coût repart « à saisir » : les lignes de coût et le total sont annulés ensemble (valeurs précédentes dans l'audit, montant dans la dépense annulée), jamais de lignes sans total ; le coût se ressaisit à la nouvelle clôture.

Une modification (PATCH) ne change jamais le statut : des dates prévues renseignées sur un BROUILLON le laissent BROUILLON jusqu'à l'action « Planifier » ; effacer le début prévu d'une PLANIFIEE est refusé (422 DATE_PREVUE_REQUISE), faute de transition PLANIFIEE → BROUILLON. Règles : apps/api/src/domain/intervention-rules.ts.
...
- **Décision** : version, updatedAt et createdById sur toute table mutée par l'API. Tables append-only sans version : AuditEvent, VehicleCompanyHistory, SettingValue, TelemetrySyncRun, échantillons. Tables globales : JobLease, WorkerHeartbeat, LoginAttempt, _prisma_migrations. Suppression physique limitée aux interventions BROUILLON jamais planifiées, aux pièces jointes temporaires et aux lots d'import non confirmés ; tout le reste est archivé ou annulé. Documenté dans docs/modele-de-donnees.md, qui fait foi pour la mise en œuvre : aucune intervention ni aucun lot d'import n'est finalement supprimé (annulation, abandon automatique après 7 jours), le contenu d'une pièce jointe temporaire est effacé du stockage et sa ligne marquée supprimée ; les suppressions physiques réellement faites par le code (données techniques expirées, fournisseur télématique en brouillon jamais synchronisé, lignes dépendantes remplacées avec leur objet, affectations à venir retirées, habilitations remplacées) y sont listées table par table et contrôlées par apps/api/src/data-model-doc.spec.ts.
